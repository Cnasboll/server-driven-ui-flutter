...
class ConstantsSet {
  ...
  /// Registers each enum value as a constant (by index) and a names array.
  void registerEnum(List<Enum> values, String namesConstant) {
    final names = <String>[];
    for (var value in values) {
      final name = _camelCaseToSnakeCase(value.name);
      names.add(name);
      registerConstant(value.index, includeIdentifier(name));
    }
    registerConstant(names, includeIdentifier(namesConstant));
  }
