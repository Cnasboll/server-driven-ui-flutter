...
import 'package:shql/tokenizer/code_span.dart';
...
class Parser {
    ...
    do {
      ...
      if (brackets != null) {
        if (brackets.symbol == Symbols.tuple && brackets.children.length == 1) {
          ...
          // If we can parse a second operand here, treat this as implicit
          // multiplication: (expr)x → expr * x, e.g. (2)ANSWER = 84.
          // The RHS must be a non-keyword or IF, to support ternaries such as
          // (3)IF FALSE THEN 2 ELSE 3, which evaluates to 9.
          if (tokenConsumer.hasNext &&
              !tokenConsumer.peek().isOperator() &&
              (!tokenConsumer.peek().isKeyword() ||
                  tokenConsumer.peek().keyword == Keywords.ifKeyword)) {
  ...
  static (ParseTree?, String?) tryParseOperand(
    ...
    String? sourceCode, [
    bool allowSign = true,
    bool tryBrackets = false,
  ]) {
    if (!tokenConsumer.hasNext) {
      return (null, 'End of token stream when expecting operand.');
    }

    // Try to parse parenthesized expressions when requested (for unary operators)
    // This handles cases like NOT (x = 5), -(x + 1), +(x + 1)
    if (tryBrackets) {
      var (brackets, _, _, bracketsError) = tryParseBrackets(
        TokenConsumer.fromParent(tokenConsumer),
        constantsSet,
        sourceCode,
      );
      if (brackets != null) {
        // If it's a tuple with a single element, extract the element
        if (brackets.symbol == Symbols.tuple && brackets.children.length == 1) {
          return (brackets.children[0], null);
        }
    ...
    if (tryConsumeSymbol(tokenConsumer, Symbols.not)) {
      var (parseTree, error) = tryParseOperand(
        ...
        sourceCode,
        false,
        true, // tryBrackets: allow NOT (expr)
      );
      if (parseTree == null && error != null) {
        return (null, error);
      }
