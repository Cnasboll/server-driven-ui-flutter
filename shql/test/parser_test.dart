...
FOR i := 1 TO 10 DO sum := 5 ERROR''';
  ...
  group('Tokenizer CRLF line number tests', () {
    test('Should correctly count lines in CRLF files', () {
      // Self-contained CRLF source: 5 lines with assignment on line 5
      final code = 'x := 1;\r\n'
          'y := 2;\r\n'
          'z := 3;\r\n'
          'w := 4;\r\n'
      ...
      var line5Tokens = <String>[];
      for (var token in parseTree.tokens) {
        if (token.startLocation.lineNumber == 5) {
          line5Tokens.add(token.lexeme);
        }
      }

      expect(line5Tokens.isNotEmpty, true,
          reason: 'Should find tokens on line 5');
      expect(line5Tokens, contains('title'));
      expect(line5Tokens, contains('_posts'));
    });
  ...
  group('Unary operators with parenthesized expressions', () {
    test('Parse NOT with parenthesized expression', () {
      var v = Tokenizer.tokenize('NOT (x = 5)').toList();
      var constantsSet = ConstantsSet();
      var p = Parser.parseExpression(v.lookahead(), constantsSet);
      expect(p.symbol, Symbols.not);
      expect(p.children.length, 1);
      expect(p.children[0].symbol, Symbols.eq);
    });

    test('Parse unary minus with parenthesized expression', () {
      var v = Tokenizer.tokenize('-(x + 1)').toList();
      var constantsSet = ConstantsSet();
      var p = Parser.parseExpression(v.lookahead(), constantsSet);
      expect(p.symbol, Symbols.unaryMinus);
      expect(p.children.length, 1);
      expect(p.children[0].symbol, Symbols.add);
    });

    test('Parse unary plus with parenthesized expression', () {
      var v = Tokenizer.tokenize('+(x + 1)').toList();
      var constantsSet = ConstantsSet();
      var p = Parser.parseExpression(v.lookahead(), constantsSet);
      expect(p.symbol, Symbols.unaryPlus);
      expect(p.children.length, 1);
      expect(p.children[0].symbol, Symbols.add);
    });

    test('Parse NOT with identifier IN list', () {
      var v = Tokenizer.tokenize('NOT (query IN _search_history)').toList();
      var constantsSet = ConstantsSet();
      var p = Parser.parseExpression(v.lookahead(), constantsSet);
      expect(p.symbol, Symbols.not);
      expect(p.children.length, 1);
      expect(p.children[0].symbol, Symbols.inOp);
    });

    test('Parse complex IF with NOT parenthesized', () {
      var code = 'IF NOT (query IN list) THEN x := 1';
      var constantsSet = ConstantsSet();
      var p = Parser.parse(code, constantsSet);
      // Parser.parse returns single statement directly, not wrapped in program
      expect(p.symbol, Symbols.ifStatement);
      // First child is the condition (NOT expression)
      expect(p.children[0].symbol, Symbols.not);
      expect(p.children[0].children[0].symbol, Symbols.inOp);
    });
  });
