...
''', isA<Map>(), [
  ...
  // STEP expr is re-evaluated every iteration: FOR i := 1 TO 20 STEP i doubles
  // the step each time (i=1→2→4→8→16, then 32>20 → overshoot → exit).
  // sum = 1+2+4+8+16 = 31.
  shqlBoth('Test for loop with dynamic step (STEP i)',
      'sum := 0; FOR i := 1 TO 20 STEP i DO sum := sum + i; sum', 31, [
      'push_const(0)',
      'store_var(SUM)',
      'load_var(SUM)',
      'pop',
